...
import WeaponRecipe from "./waffen_recipeschema.js"; // Verwende WeaponRecipe für Waffen
import connectDB from "../../../db.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

export async function weaponrecipeImport(req, res) {
  try {
    ...
    const recipeData = JSON.parse(
      await fs.promises.readFile(
        path.join(__dirname, ".", "waffen_rezepte.json"),
        "utf-8"
      )
    );

    // Überprüfen, ob bereits existierende IDs in der Datenbank vorhanden sind
    const existingRecipes = await WeaponRecipe.find({
    ...
    const insertedRecipes = await WeaponRecipe.insertMany(newRecipes);
